...
(let
  ...
  (labels
    ...
    (run-ascii (input output &aux (input-index 0))
            ...
            (flet
              ...
              (case opcode
                ...
                (2
                  ...
                  (setq pc (+ pc 4)))
                (3
                  (set-value (get-data (+ pc 1)) mode0 (char-code (char input input-index)))
                  (incf input-index)
                  (setq pc (+ pc 2)))
                (4
                  (funcall output (arg0))
  ...
  (run-ascii (format nil "~{~A~%~}"
    '(
      ...
      "OR T J"
      "RUN"))
    (lambda (value) (if (< value 128) (princ (code-char value)) (print value))))))
